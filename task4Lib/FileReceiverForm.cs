...
namespace task4Lib
{
    public partial class FileReceiverForm : Form
    {
        FileReceiver task;
        Thread fileReceive;
        public FileReceiverForm(Message msg)
        {
            InitializeComponent();

            task = new FileReceiver();
            task.Msg = msg;
            ...
            task.BlockHashed += new BlockFinishedEventHandler(file_BlockHashed);
            task.ErrorOccurred += new FileTransmissionErrorOccurEventHandler(file_ErrorOccurred);
            fileReceive = new Thread(task.Start);
            fileReceive.IsBackground = true;
            fileReceive.Start();
        }
        ...
        private void file_BlockHashed(object sender, BlockFinishedEventArgs e)
        {
